% Author: 
% Email:
% Modified by Aditya Baradwaj, Mudit Gupta
\qns{(PRACTICE) Modular Circuits}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \qitem Compare your answer with what you get when you multiplying the amplification factors of each individual voltage divider. Are they the same or different? Does this surprise you? Why or why not?
    \empt{\vspace{1cm}}
    \ans{
    ...
    \empt{\newpage}
    \qitem In order to solve this problem, we can introduce a "buffer" op-amp in between both stages of the amplifier.
    ...
    \begin{circuitikz}
        \draw (0,-2)
            ...
            to[R=$R_2$] (4,-2)
            node[ground]{};
        \draw (7,-0.5) node[op amp, yscale=-1] (opamp) {};
        \draw (opamp.out)
            to[R=$R_3$] (12,-0.5)
            node[above] {$B$}
            to[R=$R_4$] (12,-2)
...
\ans{
Using the Golden Rules of op-amps, we know that $V_+ = V_- = V_A$ in this case. Also, since the output is connected to the negative terminal, we have that $V_{out} = V_-$.
So,
\begin{flalign*}
    & V_B = \frac{R_4}{R_3 + R_4} V_out &\\
    & \implies V_B = \frac{R_4}{R_3 + R_4} \frac{R_2}{R_2 + R_1} &\\
\end{flalign*}
That's it!
}
